...
def compute_drt(freq, zexp, reg_param=1e-3, tau_density=3, integration_f_cut=None):
    ...
    # --- [수정 포인트 1] 데이터 스케일링 (정규화) ---
    # 저항값이 수십만 단위일 때 최적화 알고리즘이 헤매는 것을 방지합니다.
    scale_factor = max(float(np.max(np.abs(zexp))), 1.0)
    z_scaled = zexp / scale_factor

    # y 배열에 원본 zexp 대신 z_scaled 적용
    y_full = np.concatenate([z_scaled.real, z_scaled.imag])
    ...
    # 초기값도 스케일링된 값에 맞춤
    x0_rinf = max(1e-6, float(np.min(z_scaled.real)))
    x0 = np.zeros(n_tau + 1)
    x0[0] = x0_rinf
    ...
    # --- [수정 포인트 2] 결과 복원 ---
    # 계산이 끝난 후 다시 원래 스케일로 증폭시킵니다.
    x = res.x * scale_factor
